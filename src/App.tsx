...
function App() {
    ...
    ({ square }: SquareHandlerArgs) => {
      ...
      if (selectedSquare == null) {
        if (pieceAt && pieceAt.color === game.turn()) {
          ...
          const moves = game.moves({ square: square as ChessSquare, verbose: true }) as Array<{ to: string; flags: string }>
          setLegalSquares(moves.map((m) => m.to))
          setCaptureSquares(moves.filter((m) => m.flags.includes('c') || m.flags.includes('e')).map((m) => m.to))
        } else {
  ...
  const squareStyles = useMemo(() => {
    ...
    for (const sq of legalSquares) {
      ...
      styles[sq] = {
        ...(styles[sq] || {}),
        background: 'radial-gradient(circle, rgba(16,185,129,0.9) 20%, rgba(16,185,129,0.0) 23%)',
      }
    }
  ...
  const makeCpuMove = useCallback(() => {
    ...
    const captures = legal.filter((m) => m.flags.includes('c') || m.flags.includes('e'))
    const pool = captures.length > 0 ? captures : legal
    const choice = pool[Math.floor(Math.random() * pool.length)]
    const moved = game.move({ from: choice.from, to: choice.to, promotion: 'q' })
    if (moved) {
      setFen(game.fen())
    ...
    <div className="min-h-screen flex flex-col">
      <header className="sticky top-0 z-10 backdrop-blur bg-white/70 dark:bg-neutral-900/70 border-b border-black/5 dark:border-white/5">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src="/chess-logo.svg" alt="Logo" className="h-9 w-9 rounded-md" />
            <h1 className="text-lg font-semibold tracking-tight">The Chess App</h1>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              className="px-3 py-1.5 rounded-md border border-black/10 dark:border-white/10 text-sm"
              onClick={undoMove}
            >
              Undo
            </button>
            ...
            <div className="flex items-center gap-1 text-sm">
              ...
              <button
                className={`px-2 py-1 rounded-md border ${!playVsCpu ? 'bg-neutral-900 text-white dark:bg-white dark:text-neutral-900' : 'border-black/10 dark:border-white/10'}`}
            ...
            {playVsCpu && (
              <div className="flex items-center gap-1 text-sm">
                <span className="text-neutral-400">Play as:</span>
                <button
                  className={`px-2 py-1 rounded-md border ${playerColor === 'w' ? 'bg-neutral-900 text-white dark:bg-white dark:text-neutral-900' : 'border-black/10 dark:border-white/10'}`}
                  onClick={() => setPlayerColor('w')}
                >
                  White
                </button>
                <button
                  className={`px-2 py-1 rounded-md border ${playerColor === 'b' ? 'bg-neutral-900 text-white dark:bg-white dark:text-neutral-900' : 'border-black/10 dark:border-white/10'}`}
            ...
            <div className="h-full w-full rounded-xl border border-black/10 dark:border-white/10 overflow-hidden shadow-sm bg-white dark:bg-neutral-800">
              <Chessboard
                options={{
                  ...
                  darkSquareStyle: { backgroundColor: theme === 'light' ? '#8ab4f8' : '#394045' },
                  lightSquareStyle: { backgroundColor: theme === 'light' ? '#e9f1ff' : '#484f56' },
      ...
      {showNewGameConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/30" onClick={() => setShowNewGameConfirm(false)} />
          <div className="relative bg-white dark:bg-neutral-900 rounded-xl shadow-xl border border-black/10 dark:border-white/10 w-[92%] max-w-sm p-5">
            <div className="flex items-start gap-3">
              <div className="h-9 w-9 rounded-md bg-gradient-to-br from-amber-400 to-rose-500 flex items-center justify-center text-white text-lg">!</div>
              <div className="flex-1">
                <h3 className="font-semibold mb-1">Start a new game?</h3>
                <p className="text-sm text-neutral-500">This will clear the current position and move history.</p>
              </div>
            </div>
            <div className="mt-4 flex justify-end gap-2">
              <button className="px-3 py-1.5 text-sm rounded-md border border-black/10 dark:border-white/10" onClick={() => setShowNewGameConfirm(false)}>Cancel</button>
              <button className="px-3 py-1.5 text-sm rounded-md bg-neutral-900 text-white dark:bg-white dark:text-neutral-900" onClick={performNewGame}>New game</button>
            </div>
          </div>
        </div>
      )}
